...
/**
 * Algoritmo Extendido de Euclides para Polinômios                               
 */
...
void keygen_1998( poly h, poly fp, poly f, int N, int p, int q, int df, int dg ) {
    int qBase = 0, qExp = 0;
    logprime( &qBase, &qExp, q );
    poly fq = new_poly( N );
    poly g = new_poly( N );
    poly tmp = new_poly( N );
    random_poly_L( g, dg, dg );
    while( 1 ) {
        random_poly_L( f, df, df - 1 );
        if( inverse_prime( tmp, f, qBase  ) == 1 && inverse_prime( tmp, f, p  ) == 1 ) {
            break;
        }
    }
    inverse_prime( fp, f, p  );
    inverse_power_prime( fq, f, qBase, qExp );
    convolution( h, g, fq, q );  // h = g * fq mod q
    multiply_constant(h, p, q ); // h = p (g * fq) mod q
}

poly encrypt_1998( poly m, poly h,  int p, int q, int dr ) {
    poly c = new_poly( m.size );
    poly t = new_poly( m.size );
    poly R = new_poly( m.size );
    random_poly_L( R, dr, dr );
    copy( c, R );                          // c = R
    convolution( t, c, h, q );             // t = p * R * h mod q
    add_poly( c, t, m, q );                // c = p * R * h + m mod q
    return c;
}

poly decrypt_1998( poly c, poly f, poly fp, int p, int q ) {
    poly a = new_poly( c.size );
    poly m = new_poly( c.size );
    convolution( a, f, c, q );
    center( a, q  );
    convolution( m, fp, a, p );
    center( m, p  );
    return m;
}
...
int main(int argc, char ** argv ) {
    ...
    poly h  = new_poly( N );   // pública
    poly f  = new_poly( N );   // privada
    poly fp = new_poly( N );   // privada
    poly m = new_poly( N );    // mensagem
    ...
    keygen_1998( h, fp, f, N, p, q, df, dg );
    
    
    poly c = encrypt_1998(m, h, p, q, dr );
    print( c );
    
    
    poly m2 = decrypt_1998( c, f, fp, p, q );
